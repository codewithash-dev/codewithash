...
export default function InstagramPostCard({ post, onUpdate }: { post: Post; onUpdate: () => void }) {
  ...
  return (
    <div className="bg-black border border-gray-800 rounded-sm mb-6 max-w-[470px] mx-auto">
      {/* Header */}
        ...
        <Link href={`/projects/social-media/profile/${post.user_id}`} className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 rounded-full p-[2px]">
            <div className="w-full h-full bg-black rounded-full flex items-center justify-center">
              <span className="text-white text-sm">{post.profiles?.username?.[0]?.toUpperCase()}</span>
            </div>
          </div>
          <span className="text-white font-semibold text-sm">{post.profiles?.username}</span>
        </Link>
        {isOwnPost && (
          <button onClick={handleDelete} className="text-white hover:text-gray-400">
      ...
      <div className="p-3">
        ...
        {/* Likes */}
        <p className="text-white font-semibold text-sm mb-2">{likesCount} likes</p>

        {/* Caption */}
        <p className="text-white text-sm">
          <Link href={`/projects/social-media/profile/${post.user_id}`} className="font-semibold mr-2">
        ...
        {post.comments_count > 0 && (
          <Link href={`/projects/social-media/post/${post.id}`} className="text-gray-400 text-sm mt-1 block">
